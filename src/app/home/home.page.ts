...
export class HomePage {
				...
				this.global.httpGetF('https://telephonekhooneh.com/other/wp-json/wp/v2/mag').subscribe(
					(res) => {
						...
						res.map((blog) => {
							const bl = new Blog();
							// bl.created_at = blog.created_at;
							bl.created_at = this.getJalaliDate(blog?.date);
							bl.id = blog.id;
							bl.link = blog?.link;
							bl.title = blog?.title?.rendered;
							bl.media = blog?.yoast_head_json?.og_image[0]?.url;
	...
	getJalaliDate(date) {
		...
		return jalaali(splitDate[0], 'YYYY-M-D').format('jYYYY/jMM/jD');
		//return {};
	}
